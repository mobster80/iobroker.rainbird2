...
const rainbirdNative = require('./lib/rainbird-native');
...
function main() {
    ...
    // Use native JavaScript implementation (ported from pyrainbird)
    adapter.log.info('[INFO] Using native JavaScript implementation (pyrainbird protocol)');
    controller = new rainbirdNative.RainbirdController(deviceIpAdress, devicePassword, adapter);
    
    // Legacy implementation available as fallback
    const useLegacy = adapter.config.useLegacy === true;
    if (useLegacy) {
        adapter.log.info('[INFO] Fallback to legacy JavaScript implementation');
